...
(define run-trigint-tests
  -> (let Ign0 (demo-register-calculus)
          ...
          \\ --- power reductions Sin^2/Cos^2: verify by DIFFERENTIATE-BACK ---
          \\ The merged engine has the coeff-aware Pythagorean fold, so D[antideriv]
          \\ - integrand reduces to 0 (more rigorous than an exact structural match,
          \\ and robust to the slightly-varying collected form -1*..*1/2 vs -1/2*..
          \\ that accumulated harness state can produce -- both are correct).
          B1 (tg-check-db "Sin[x]^2" (tg-pow (tg-sin) 2))
          B2 (tg-check-db "Cos[x]^2" (tg-pow (tg-cos) 2))
