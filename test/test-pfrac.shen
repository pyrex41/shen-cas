...
(define pf-eval
  [int N] -> [some (pf-i2f N)]
  [rat N D] -> [some (pf-fdiv (pf-i2f N) (pf-i2f D))]
  [[sym S] | Args] -> (pf-eval-app (str S) Args)
  _ -> [none])

(define pf-i2f N -> (+ N 0.0))

(define pf-fdiv A B -> (if (pf-fzero? B) (error "pf-eval: /0") (/ A B)))
...
(define pf-eval-divide-2
  [some X] [some Y] -> [some (pf-fdiv X Y)]
  _ _ -> [none])
...
(define pf-sqrt-iter
  X G 0 -> G
  X G N -> (pf-sqrt-iter X (* 0.5 (+ G (pf-fdiv X G))) (- N 1)))
